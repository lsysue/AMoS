...
def classification_train_epoch(train_loader, model, 
    ...
    for batch_idx, (inputs, ctargets, ttargets, indices) in tqdm(enumerate(train_loader)):
        ...
        for b in range(batch_size):
            ...
            anchor_indice = indices[b, 0:1].repeat(1, n_edges).view(-1)
            candidate_indices = indices[b, batch_indices + 1].view(-1)
            row_indices = torch.cat((row_indices, indices[b, 0:1].repeat(1, n_edges).view(-1)), dim=0)
            col_indices = torch.cat((col_indices, indices[b, batch_indices + 1].view(-1)), dim=0)
            data = torch.cat((data, batch_pred_labels.view(-1)), dim=0)
...
def train(args, cfg):
    ...
    for epoch in range(start_epoch + 1, cfg.TRAIN.EPOCHS + 1):
        ...
        if (is_master_proc):
            ...
            # adj_matrix = classification_train_epoch(train_dataloader, main_net, criterion, optimizer, epoch, cfg, device, output_path, is_master_proc)
            ...
            if cfg.ITERCLUSTER.METHOD in ['finch', 'advfinch']:
                train_cluster_labels, n_train_cluster, iter_adj_matrix = cluster.forward(train_data, distance=iter_multimodal_distance)
                train_cluster_labels = train_cluster_labels[:, cfg.ITERCLUSTER.FINCH_PARTITION]
            elif cfg.ITERCLUSTER.METHOD == 'nncluster':
                train_cluster_labels, n_train_cluster, iter_adj_matrix = cluster.forward(train_data, init_adj_matrix)
                train_cluster_labels = train_cluster_labels[:, cfg.ITERCLUSTER.FINCH_PARTITION]
            elif cfg.ITERCLUSTER.METHOD in ['kmeans', 'dbscan', 'optics']:
                train_cluster_labels = cluster.fit(train_data).labels_
                train_cluster_labels = torch.tensor(train_cluster_labels)


            ri, ari, nmi, ami, p, r, f_beta = evaluate_clustering(true_labels, train_cluster_labels)
            print(f'==> Metrics of clustering:{ri}, {ari}, {nmi}, {ami}\n==> {p}, {r}, {f_beta}')
            with open(os.path.join(output_path, 'train_cluster_metrics.txt'), "a") as f:
        ...
        if is_master_proc:
            # Update embeding
            # adj_matrix = classification_val_epoch(val_dataloader, main_net, criterion, epoch, cfg, device, output_path, is_master_proc)
            ...
            if cfg.ITERCLUSTER.METHOD in ['finch', 'advfinch']:
                iter_cluster_labels, n_iter_cluster, iter_adj_matrix = cluster.forward(iter_data, distance=iter_multimodal_distance)
                iter_cluster_labels = iter_cluster_labels[:, cfg.ITERCLUSTER.FINCH_PARTITION]
            elif cfg.ITERCLUSTER.METHOD == 'nncluster':
                iter_cluster_labels, n_iter_cluster, iter_adj_matrix = cluster.forward(iter_data, init_adj_matrix)
                iter_cluster_labels = iter_cluster_labels[:, cfg.ITERCLUSTER.FINCH_PARTITION]
            elif cfg.ITERCLUSTER.METHOD in ['kmeans', 'dbscan', 'optics']:
